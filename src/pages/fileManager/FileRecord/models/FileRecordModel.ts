...
import {fileRecordPageGetList, removeFile} from "@/pages/fileManager/FileRecord/service";


export default function FileRecordModel() {
  const [fileRecordList, setFileRecordList] = useState<FileRecordEntity[]>([]);
  ...
  // 查询
  const getFileRecord = useCallback((queryParams: QueryFileRecord) => {
    setLoading(true);
    fileRecordPageGetList(queryParams).then((res) => {
      setLoading(false);
      if(res.code === 200){
        setHasError(false);
        setFileRecordList(res.data?.content || []);
  ...
  // 删除用户
  const delFile = useCallback(async (fileId: string): Promise<boolean> => {
    setLoading(true);
    return await removeFile(fileId).then(res => {
      setLoading(false);
      if(res.code === 200){
        setHasError(false);
