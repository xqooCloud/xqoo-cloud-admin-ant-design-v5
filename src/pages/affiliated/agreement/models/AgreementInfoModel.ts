...
import {
  ...
  getAgreementInfo,
  deleteAgreement
} from "@/pages/affiliated/agreement/service";
...
export default function UserLoginHistoryModel() {
  ...
  // 查询
  const getAgreementFromServer = useCallback((queryParams: QueryAgreementInfo) => {
    setLoading(true);
    getAgreementInfo(queryParams).then((res) => {
      setLoading(false);
      if(res.code === 200){
        setHasError(false);
        setAgreementList(res.data?.content || []);
  ...
  // 删除
  const deleteAgreementToServer = useCallback(async (key: string) => {
    setLoading(true);
    return await deleteAgreement(key).then(res => {
      setLoading(false);
      if(res.code === 200){
        setHasError(false);
  ...
  // 新增/修改
  const updateAgreementToServer = useCallback(async (data: AgreementInfo) => {
    setLoading(true);
    return await updateAgreement(data).then(res => {
      setLoading(false);
      if(res.code === 200){
        setHasError(false);
  ...
  return {
    ...
    deleteAgreementToServer,
    updateAgreementToServer,
    alertTipsHandle
  }
}
